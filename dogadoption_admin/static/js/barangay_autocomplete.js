(function () {
    ...
    function findMatches(items, value) {
        ...
        const direct = items.filter((item) => normalizeText(item).includes(normalized));
        if (direct.length) {
            return direct;
        }

        // Fuzzy suggestions allow slight misspellings like "Vilarreal" -> "Villareal".
        const threshold = normalized.length >= 7 ? 3 : 2;
        return items
            .map((item) => {
                const itemNormalized = normalizeText(item);
                return {
                    item,
                    distance: levenshteinDistance(normalized, itemNormalized),
                };
            })
